...
async def extract_from_file_or_url(input_file_or_url, output_file, research_topic, summary=None):
    print("input_file_or_url IS")
    print(input_file_or_url)
...
async def async_main(context):
    ...
    async with ClientSession() as session:
        for url in urls:
            clean_url = url["metadata"]["url"].replace("https://", "").replace('http://', '').replace("www.", "").replace(':', '').replace(';', '').replace("/", "_").replace("?", "_").replace("=", "_").replace("&", "_")
            filepath = project_dir + "/" + clean_url + ".csv"

            print("filepath")
            print(filepath)
            task = extract_from_file_or_url(url["metadata"]["url"], filepath, context["research_topic"], context.get("summary", None))
            tasks.append(task)
    ...
    for url in urls:
        print("url")
        print(url)
        update_url_entry(url["metadata"]["url"], url["document"], crawled=True)
    return context
