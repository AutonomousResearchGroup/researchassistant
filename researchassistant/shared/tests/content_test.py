...
from researchassistant.shared.content import (
...
def test_get_content_from_pdf_invalid_pdf_content():
    ...
    assert os.path.exists(temp_file_path), "The temporary file should exist."
    content_result = get_content_from_pdf(temp_file_path)
    expected_result = "Failed to read PDF file."
    assert content_result == expected_result, "The content result should be 'Failed to read PDF file.'."
...
def test_get_content_from_pdf_success():
    ...
    content_result = get_content_from_pdf(temp_file_path)
    expected_result = ""
    assert content_result == expected_result, "The content result should be an empty text."
...
def test_get_content_from_txt_success():
    ...
    assert content_result == expected_result, "The content result should be 'This is a test content.'."
...
def test_get_content_from_file_http():
    ...
    assert type(content_result) == str, "The content result type should be a string."
...
def test_get_content_from_file_invalid_file_format():
    ...
    assert content_result == expected_result, \
        "The content result should be 'Invalid input file format. Please provide a URL or a PDF file.'."
